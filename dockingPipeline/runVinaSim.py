...
if __name__ == "__main__":
    for filename in os.listdir(pdbDir):
        #print filename[:-4]
        os.system('~/Desktop/Tools/vina --cpu 2 --receptor '+inputdir+filename[:-4]+'.receptor.pdbqt --ligand '+inputdir+filename[:-4]+'.ligand.pdbqt --out '+resultDir+filename[:-4]+'.ouput.pdbqt --config '+inputdir+filename[:-4]+'.conf')
